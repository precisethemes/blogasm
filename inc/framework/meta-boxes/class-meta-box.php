<?php
/**
 * Blogasm function for rendering meta-boxes in single post/page area
 *
 * @package Blogasm
 */
...
/*----------------------------------------------------------------------
# Start Class Blogasm_Post_Global_Meta_Box
-------------------------------------------------------------------------*/
if ( ! class_exists( 'Blogasm_Post_Global_Meta_Box' ) ) {
    class Blogasm_Post_Global_Meta_Box {
        ...
        public function load_scripts( $hook ) {
            ...
            // Enqueue Style
            wp_enqueue_style( 'blogasm-metabox-style', BLOGASM_THEME_URI .'/assets/back-end/css/metabox-style' . $min . '.css', false, BLOGASM_THEME_VERSION, 'all' );

            // Enqueue metabox js
            wp_enqueue_script( 'blogasm-metabox-script', BLOGASM_THEME_URI . '/assets/back-end/js/metabox-script' . $min . '.js', array( 'jquery' ), BLOGASM_THEME_VERSION, true );
        ...
        public function post_meta( $post ) {
            ...
            add_meta_box(
                'post_meta_fields',
                $obj->labels->singular_name . ' '. esc_html__( 'Settings', 'blogasm' ),
        ...
        public function display_meta_box( $post ) {
            ...
            wp_nonce_field( basename( __FILE__ ), 'blogasm_meta_nonce' );
            ...
            // Empty notice
            $empty_notice = '<p>'. esc_html__( 'No meta settings available for this post type or user.', 'blogasm' ) .'</p>';
                ...
                <div class="metabox-settings-tabs">
                    <ul class="metabox-tab-nav">
                        ...
                        foreach ( $active_tabs as $tab ) {
                            $tab_count++;
                            // Define tab title
                            $tab_title = $tab['title'] ? $tab['title'] : esc_html__( 'Other', 'blogasm' ); ?>
        ...
        public function save_meta_data( $post_id ) {
            ...
            if ( ! isset( $_POST['blogasm_meta_nonce'] ) || ! wp_verify_nonce( sanitize_key( $_POST['blogasm_meta_nonce'] ), basename( __FILE__ ) ) ) {
                return;
            }
        ...
        private function meta_array( $post = null ) {
            ...
            // Default variable
            $default = esc_html__( 'Default', 'blogasm' );

            // Sidebar Layout Tab
            $array['sidebar']  = array(
                'title'     => esc_html__( 'Sidebar Layout', 'blogasm' ),
                'settings'  => array(
                    'sidebar_layout'    =>array(
                        'title'         => esc_html__( 'Sidebar Layout', 'blogasm' ),
                        'description'   => esc_html__( 'Default value is inherit from single post or page settings.', 'blogasm' ),
                        'id'            => $prefix . 'sidebar_layout',
                        'type'          => 'radio',
                        'options'       => blogasm_content_layouts( array( 'default-sidebar' => $default )),
            ...
            $array['featured_img_type']  = array(
                'title'             => esc_html__( 'Featured Image Type', 'blogasm' ),
                'settings'          => array(
                    'subtitle'      => array(
                        'title'         => esc_html__( 'Image Type', 'blogasm' ),
                        'description'   => esc_html__( 'Select the featured image type you want to display on Single Post and Blog Posts listing.','blogasm' ),
                        'id'            => $prefix . 'featured_img_type',
                        'type'          => 'radio',
                        'default'       => 'portrait-img',
                        'options'       => array(
                            'portrait-img'      => esc_html__( 'Portrait Image', 'blogasm' ),
                            'landscape-img'     => esc_html__( 'Landscape Image', 'blogasm' ),
                            'full-width-img'    => esc_html__( 'Full Width', 'blogasm' ),
                        ),
                    ),
                ),
            );
            ...
            return apply_filters( 'blogasm_metabox_array', $array, $post );
        }
    }
    ...
    if ( is_admin() ) {
        ...
        new Blogasm_Post_Global_Meta_Box;
    }
}
